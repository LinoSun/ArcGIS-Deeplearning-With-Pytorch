...
from datasets import train_transform, val_transform, DXMDataset
...
base_dataset = DXMDataset(data_path, transforms=train_transform)
...
# model = EfficientNet.from_name(model_name).to(device)
model = get_efficientunet_b4(out_channels=2).to(device)

save_dir = r'F:\deepLearning\data\dongxiaomai\xiaomai_samplearea_tra30_4band\models\baseline0104'
# 模型保存路径
save_ckpt_dir = os.path.join(save_dir, model_name, 'ckpt')
save_log_dir = os.path.join(save_dir, model_name)
...
# 训练
best_model, model = train_net(param, model, base_dataset)
